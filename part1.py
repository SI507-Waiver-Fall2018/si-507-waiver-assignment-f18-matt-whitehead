...
def pop_contest(counts_dict):
    return sorted(counts_dict.items(), key = lambda x: (-x[1], x[0]))
popular_nouns = pop_contest(noun_counts)
popular_verbs = pop_contest(verb_counts)
